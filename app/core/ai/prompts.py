...
- hidden_thought_process: Твой внутренний монолог.
...
class ChunkCultivationPrompts:
    ...
    def build_product_cultivation_prompt(
        ...
        ЗАДАЧА ИИ:
        1. Проанализировать товар на основе сырых объявлений
        2. Определить target_status (COMMODITY, SPECIALTY, PREMIUM, NICHE)
        3. Выдать price_analysis с точными цифрами
        4. Дать описание + рекомендации
        
        КРИТИЧНО: Используй только РЕАЛЬНЫЕ данные из items, не выдумывай!
        """
        
        # Готовим примеры объявлений (первые 15 для контекста)
        items_preview = []
        ...
        {{
          ...
          "content": {{
            ...
            "price_analysis": {{
              "min": <число из math_block>,
              "max": <число из math_block>,
              "avg": <число из math_block>,
              "med": <число из math_block>,
              "count": <число из math_block>,
              "interpretation": "Описание диапазона цен в прозе"
            }},
            "target_audience": "Кто основной покупатель",
            "recommendations": [
              "Рекомендация 1",
              "Рекомендация 2",
              "Рекомендация 3"
          ...
          "summary": "Однострочный summary для быстрого чтения"
        }}
        ...
        - Если данных мало (<5 объявлений), установи confidence < 0.8
    ...
    def build_category_cultivation_prompt(
        ...
        ЗАДАЧА ИИ:
        1. Проанализировать категорию как группу товаров
        2. Найти паттерны, тренды, особенности
        3. Описать рыночную ситуацию в категории
        4. Дать рекомендации для навигации по категории
        """
        ...
        prompt = f"""ТЫ — АНАЛИТИК РЫНКА. КУЛЬТИВАЦИЯ КАТЕГОРИИ.
    ...
    def build_database_cultivation_prompt(
        ...
        ЗАДАЧА ИИ:
        1. Проанализировать состояние БД
        2. Описать основные статистики
        3. Выявить ключевые слова и темы
        4. Дать рекомендации по расширению
        """
    ...
    def build_ai_behavior_cultivation_prompt(
        ...
        ЗАДАЧА ИИ:
        1. Проанализировать логи действий
        2. Выявить правила self-correction
        3. Вычислить эффективность анализа
        4. Дать подсказки для улучшения
        """
        ...
        - effectiveness_score (0-1) — твоя честная оценка качества
        """
